...
export async function cleanModeratorsChannel(event: APIGatewayEvent) {
  const history = await slackClient.paginate("conversations.history", {
    channel: process.env.MODERATORS_CHANNEL_ID
  });
  ...
  for await (let page of history) {
    ...
    for await (let question of questions) {
      if (!question.hasAnswers || question.hasVerifiedAnswers) {
  ...
  return "Messages have been deleted successfully.";
}